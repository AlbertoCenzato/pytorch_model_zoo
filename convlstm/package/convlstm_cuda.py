...
class ConvLSTMFunction(torch.autograd.Function):
    ...
    def forward(ctx, input, weights, bias, old_h, old_cell):
        outputs = convlstmcuda.forward(input, weights, bias, old_h, old_cell)
    ...
    def backward(ctx, grad_h, grad_cell):
        outputs = convlstmcuda.backward(grad_h.contiguous(), grad_cell.contiguous(), *ctx.saved_tensors)
        d_old_h, d_input, d_weights, d_bias, d_old_cell = outputs
        return d_input, d_weights, d_bias, d_old_h, d_old_cell


class ConvLSTMCudaCell(nn.Module):
